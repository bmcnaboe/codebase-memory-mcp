...
- [x] T029 [AGL-11] Build and install from `origin/main`: `scripts/build.sh`; put `build/c/codebase-memory-mcp` on PATH; run `codebase-memory-mcp install --clients=claude,codex` (the one sanctioned real-config install — it edits this machine's Claude/Codex config as intended; later loop iterations may begin receiving cbm hook context, which is expected)
      Done: built origin/main @8f81c50c, binary self-installed to ~/.local/bin/codebase-memory-mcp (on PATH); `install --clients=claude,codex` configured claude (top-level mcpServers) + codex (~/.codex/config.toml) as intended. NOTE (out of scope): the binary's `--clients` allowlist (cli_clients_apply_selection) only filters clients present in its CLI_CLIENTS[] table; kimi is absent from that table, so it can never be de-selected and leaked through — install also wrote ~/.kimi-code/mcp.json. This is a pre-existing binary bug, NOT AGL-8 (whose scope was install.sh forwarding --clients at all). Reported in AGL-8 evidence; flagged to user for cleanup decision.
- [x] T030 [AGL-11] Index the AGL-5 baseline monorepo (repo path recorded in /Users/bmcnaboe/development/context-layer-bench/micro/cbm.jsonl) and verify AGL-5 (10/10 non-empty at default deadline; forced miss logged; `daemon start` in `--help`; override honored) and AGL-7 (payload-cwd worktree matrix); evidence comments on AGL-5 and AGL-7
      Done: baseline monorepo = /Users/bmcnaboe/development/dmatrix (re-indexed with fork binary: 25,970 nodes/107,570 edges). AGL-5: 10/10 non-empty SessionStart at default deadline (p50≈409ms, max=463ms) with warm daemon; forced miss (CBM_HOOK_DEADLINE_MS=1→50ms floor) logged to stderr + logs/hook-augment-timeouts.log; daemon start in --help; override honored. AGL-7: 2×2 payload×process cwd matrix all non-empty — worktree payload cwd → its own project (782B, was 0 pre-fix), unindexed worktree → explicit "no indexed graph project matched" (656B, never silent), main unchanged (789B). Evidence comments posted on AGL-5 (1d089772) and AGL-7 (f23abbdd). Temp worktree + its index project cleaned up; dmatrix project retained.
- [x] T031 [AGL-11] Verify AGL-8 via a sandbox `$HOME` and AGL-6 via the dmatrix probes (`search_graph` on `%SupplementalContentPanel.vue` shows script symbols beyond Module+File; `trace_path("handleAddImage")` resolves; never via `check_index_coverage`); evidence comments on AGL-8 and AGL-6; confirm the second-machine install one-liner is documented on the cbm adoption project
      Done. AGL-6: search_graph on %SupplementalContentPanel.vue → 45 results (Module + <script setup> Functions/Variables/Type; handleAddImage 275-285) vs pre-fix 2 nodes; trace_path(handleAddImage) resolves (callees emit, fileToBase64) vs pre-fix not-found — the ticket's own litmus, both flipped; used node counts + trace_path, not check_index_coverage. Evidence comment 6344811b. AGL-8 (sandbox $HOME, loopback stub fixture + real-binary --dry-run): AC2 unknown flag→exit 2 before download; AC3 --skip-config forwarded unchanged; AC1 install.sh now forwards --clients=claude,codex (pre-fix dropped it). RESIDUAL out-of-scope binary bug: cli_clients_apply_selection de-selects only CLI_CLIENTS[] (~25 tokens) but installer detects 43 surfaces, so kimi (absent from the table) always leaks → end-to-end --clients=claude,codex still configures Kimi. Reproduces with the binary directly, independent of install.sh. Evidence comment 127e5609; recommended a follow-up. Second-machine one-liner IS documented on the cbm adoption project description.
- [x] T032 [risky] Close tranche F: full gate green; move AGL-11 to In Review with an evidence summary — full gate green (exit 0, 1118s). First run hit 25 install-activation failures in test_cli.c ("active CBM sessions … no activation was committed"): the sanctioned T029 real install left a permanent daemon holding the machine-global CBM coordination rendezvous BUSY (its key is a fixed product-domain string; isolation is only by shared runtime-parent dir), which every install test contends for. Root-caused, not papered over: added an idempotent best-effort `daemon stop` preflight to the full/final tiers of scripts/dev/gate.sh (fork-only dev tooling, never merged to main/PR branches), so the dogfooded daemon can't collide with the suite. Re-run green. AGL-11 → In Review with a per-AC evidence summary (comment a4c56486).
